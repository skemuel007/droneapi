...
{
    private const string _alias = "droneappauditlog";
    ...
    public AuditTrailProvider(IOptions<AuditTrailOptions> auditTrailOptions,
        IOptions<ElasticSearchSettings> elasticSearchOptions)
    {
        ...
        var pool = new StaticConnectionPool(new List<Uri> { new Uri(_elasticSearchSettings.Uri) });
    ...
    public void AddLog(T auditTrailLog)
    {
        ...
        if (!response.IsValid)
        {
            throw new ElasticsearchClientException($"Add audit log disaster! {response.DebugInformation}");
        }
    }
